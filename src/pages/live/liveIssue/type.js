...
class Banner extends Component{
    ...
    //跳转到添加页面
    goAddBanner(){
        this.props.history.push(`/live/liveEdit/type/detail`);
    }
    //点击查看图标
    clickCheck(id,name){
    ...
    clickEdit(item){
        this.props.history.push({
            pathname:`/live/liveEdit/type/detail/${item.id}/?checked=1&name=${item.name}`
        })
    }
    //上线
    clickOnline(item){
                       ...
                       {this.state.dataList.map((item,index)=>{
                           return (
                               <tr key={index}>
                                   ...
                                   <td>
                                        <Link className='gl-link' to={`/live/liveIssue/type/list/${item.id}/?name=${item.name}`} >
                                        <Icon type="link" />
                                        关联文件
                                        </Link>
                                   </td>
