...
class Banner extends Component{
    ...
    //跳转到添加页面
    goAddBanner(){
        this.props.history.push(`/service/serviceEdit/type/detail`);
    }
    //点击查看图标
    clickCheck(id,name){
    ...
    clickEdit(item){
        this.props.history.push({
            pathname:`/service/serviceEdit/type/detail/${item.id}/?checked=1&name=${item.name}`
        })
    }
    //上线
    clickOnline(item){
        ...
        return (
            <div className={style.container}>
                ...
                <div className={style.content}>
                    ...
                    <div className={style.handle + ' clearfix'}>
                        ...
                        <div className='fr'>
                            <Search
                                ...
                                style={{ width: 350 }}
                            />
                            {selectValue != '4' ? <IssueButton callback={()=>{this.loadList();this.setState({issueList:[]})}} type={10} dataList ={this.state.issueList} />:null }
                    ...
                    <TableList
                        tdHeight='58px'
                        thead={[{checked:()=>{this.checkboxAll()},isChecked:this.state.dataList.length == this.state.issueList.length},{width:'5%',name:' '},{width:'20%',name:'类型名称'},{width:'30%',name:'创建时间'},{width:'20%',name:'操作'},{width:'20%',name:'管理'}]}
                    >
                       {this.state.dataList.map((item,index)=>{
                           return (
                               <tr key={index}>
                                   ...
                                   <td>
                                        <Link className='gl-link' to={`/service/serviceIssue/type/list/${item.id}/?name=${item.name}`} >
                                        <Icon type="link" />
                                        关联文件
                                        </Link>
                                   </td>
