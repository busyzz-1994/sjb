...
class Line extends Component{
    ...
    render(){
        let {xAxisData,reg,newReg,active} = this.props;
        var option = {
            tooltip: {
                trigger: 'axis'
            },
            legend: {
                data:['注册用户数','新增注册数','活跃用户数']
            },
            grid: {
                left: '3%',
                right: '4%',
...
class N1 extends Component{
    ...
    loadData(){
        ...
        api.getN2({startTime,endTime,dateType}).then(res=>{
            let regList = res[0];
            api.getN2_2({startTime,endTime,dateType}).then(res=>{
                let activeList = res[0];
                let {reg,newReg,active} = formatUtil.getData(xAxisData,regList,activeList);
                this.setState({reg,newReg,active})
            })
    ...
    render(){
        let {dateType,startTime,endTime,format,type,xAxisData,reg,newReg,active} = this.state;
        return (
            <div className={style.panelItem}>
                ...
                <div className={style.panelBody}>
                    <div>
                        <Line reg={reg} active={active} newReg={newReg} xAxisData={xAxisData} />
                    </div>
                </div>
            </div>
        )
